...
function processDuplicateFree(list, callback) {
  return callback(list.filter((x, index, arr) => arr.indexOf(x) == index));
}
...
function getFullNames(arr) {
  ...
  arr.forEach(x => 
    {
    newArr.push(`${x.last_name}, ${x.first_name}`);
  });
  return newArr;
}
...
function firstNamesAllCaps(arr) {
  return arr.map(x => x.first_name.toUpperCase());
}
...
function tallyUpDonations(arr) {
  return arr.reduce((acc, x) => acc + x.donation, 0);
}
